...
<script>
    ...
    upgradeButton.addEventListener('click', () => {
        if (points >= upgradeCost) {
            ...
            upgradeCost = Math.floor(upgradeCost * 1.35); // Increase upgrade cost by 35%
            updateDisplay();
    ...
    // Block "Enter" key from triggering points
    document.addEventListener('keydown', (event) => {
        if (event.key === "Enter") {
            event.preventDefault(); // Prevent the default behavior of the Enter key (like submitting forms or reloading the page)
        }
    });
